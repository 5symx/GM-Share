#ifndef GMS_CUDA_UTIL_H
#define GMS_CUDA_UTIL_H
...
#include <gms/cutlass/cutlass_struct.h>
    ...
    for (auto &name_and_params : kernel_params) {
        ...
        if (original_kernel_map.find(host_func) == original_kernel_map.end()) {
            ...
            std::vector<std::string> transform_kernel_names {
                kernel_name,
                kernel_name + "_gms_ptb",
                kernel_name + "_gms_dynamic_ptb",
                kernel_name + "_gms_preemptive_ptb",
                kernel_name + "_gms_sliced"
            };
            ...
            for (int i = 0; i < transform_kernel_names.size(); i++) {
                ...
                if (transform_kernel_name == kernel_name + "_gms_ptb") {
                    num_params += 1;
                } else if (transform_kernel_name == kernel_name + "_gms_dynamic_ptb") {
                    num_params += 3;
                } else if (transform_kernel_name == kernel_name + "_gms_preemptive_ptb") {
                    num_params += 4;
                } else if (transform_kernel_name == kernel_name + "_gms_sliced") {
                    num_params += 2;
                }
