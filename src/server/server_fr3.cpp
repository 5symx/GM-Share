...
#include <gms/transform.h>
#include <gms/util.h>
#include <gms/ipc_util.h>
#include <gms/cuda_util.h>
#include <gms/log.h>
#include <gms/msg_struct.h>
#include <gms/cache.h>
#include <gms/cache_util.h>
#include <gms/generated/cuda_api.h>
#include <gms/generated/msg_struct.h>
#include <gms/generated/server.h>

#include <cuda_profiler_api.h>

GmsServer *GmsServer::server = new GmsServer();
...
void GmsServer::start_main_server() {

    cudaProfilerStart();

    iox::runtime::PoshRuntime::initRuntime(APP_NAME);
    iox::popo::UntypedServer handshake_server({"Gms", "handshake", "event"});

    implicit_init_cuda_ctx();

    GMS_SPD_LOG_ALWAYS("Gms server is up ...");
    ...
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        handshake_server.take().and_then([&](auto& requestPayload) {
            ...
            {
                // auto channel_desc_str = std::string("Gms-Communication") + ;
                ...
                worker_servers[mapped_id] = new iox::popo::UntypedServer({channel_desc, "gms", "gms"});
                // std::thread t(&GmsServer::start_worker_server, GmsServer::server, client_id, mapped_id);
            ...
            else
            {
                ...
                // std::thread t(&GmsServer::reset_worker_server, GmsServer::server, client_id, mapped_id);
        ...
        // Check whether any worker thread has exited. Free its resources. Only free memory
    ...
    cudaProfilerStop();
    GmsCache::cache->save_transform_cache();
    
}

int32_t GmsServer::get_client_priority(int32_t client_id)
{
...
int32_t GmsServer::get_client_stream_priority(int32_t client_id)
{
...
void GmsServer::increment_client_queue_size(int32_t client_id)
{
    client_data_all[client_id].queue_size++;
}

void GmsServer::start_worker_server(int32_t client_id, int32_t mapped_id) {
    ...
    if (policy == GMS_SCHEDULER_POLICY::PRIORITY) {
    ...
    GMS_SPD_LOG_ALWAYS("Gms worker server is up ...");

    auto process_name = get_process_name(client_id);
    GMS_SPD_LOG_ALWAYS("Client process: " + process_name);

    if (policy == GMS_SCHEDULER_POLICY::PRIORITY) {
        int priority = get_client_priority(client_id);
        GMS_SPD_LOG_ALWAYS("Client priority: " + std::to_string(priority));
    }
    ...
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        worker_server->take().and_then([&](auto& requestPayload) {
            ...
            // GMS_SPD_LOG_VALUE("Processing init request for on worker for client_id: {}", client_pid_of_request);
    ...
    GMS_SPD_LOG_ALWAYS("Gms worker server has exited ...");
    // cudaProfilerStop();

}

void GmsServer::reset_worker_server(int32_t client_id, int32_t mapped_id_reset) {

    auto &client_meta = client_data_all[client_id];

    GMS_SPD_LOG_ALWAYS("Gms worker server from warm to up ...");

    auto process_name = get_process_name(client_id);
    GMS_SPD_LOG_ALWAYS("Current Client process: " + process_name);
    ...
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        worker_server->take().and_then([&](auto& requestPayload) {
            ...
            // GMS_SPD_LOG_VALUE("Processing reset request on worker for client_id: {}", client_id);
...
void GmsServer::launch_and_measure_kernel(KernelLaunchWrapper &kernel_wrapper, int32_t client_id,
                                            ...
                                            std::vector<CudaLaunchConfig> alternative_configs, bool is_preemptive)
{
    ...
    for (auto &config : configs) {
        ...
        if (metrics.count >= KERNEL_PROFILE_ITERATIONS) {
            ...
            if (norm_speed >= 2.) {
                ...
                GMS_SPD_WARN("Detecting abnormal norm_speed of " + std::to_string(norm_speed) + " of kernel " + kernel_str);
                GMS_SPD_WARN("Deleting measurement for original config.");
                return;
            }
            ...
            if (config.max_worker_blocks < CUDA_NUM_SM) {
                ...
                GMS_SPD_WARN("Setting max_worker_blocks to " + std::to_string(config.max_worker_blocks) + " for " + kernel_str);
            }
        }
        ...
        for (auto &config : alternative_configs) {
            ...
            if (metrics.count >= KERNEL_PROFILE_ITERATIONS) {
                ...
                if (norm_speed >= fallback_threshold) {
                    GMS_SPD_WARN("Fall back to use alternative config.")
                    break;
                }
            }
    ...
    if (fallback_to_original || configs.empty()) {
        ...
        if (configs.empty()) {
            kernel_wrapper.kernel_to_dispatch(base_config, nullptr, nullptr, nullptr, false, 0, nullptr, nullptr, 0, true);
            GMS_SPD_WARN("No transformed config is provided. Will Use original config.")
        } else {
            GMS_SPD_WARN("Fall back to original config as transformed kernel norm speed is below threshold: " + std::to_string(best_norm_speed));
        }
    ...
    GMS_SPD_LOG_ALWAYS("Tuning complete for: " + kernel_str);

    for (auto &config : configs) {
        auto res = get_single_kernel_perf(launch_call, config, &found);
        auto norm_speed = res.metrics.norm_speed;
        auto latency_ms = res.metrics.latency_ms;
        GMS_SPD_LOG_ALWAYS("\tLaunch config: " + config.str() + ". Latency: " + std::to_string(latency_ms) + " ms Norm speed: " + std::to_string(norm_speed));
    }
    GMS_SPD_LOG_ALWAYS("Best config: " + best_config.str() + ". Latency: " + std::to_string(best_latency_ms) + " ms Norm speed: " + std::to_string(best_norm_speed) + "\n");

    return;
}

void GmsServer::tune_kernel_launch(KernelLaunchWrapper &kernel_wrapper, int32_t client_id, std::vector<CudaLaunchConfig> &configs)
{
    auto &launch_call = kernel_wrapper.launch_call;
    auto &client_data = client_data_all[client_id];
    auto kernel_name = host_func_to_demangled_kernel_name_map[launch_call.func];
    ...
    GMS_SPD_LOG_ALWAYS("Launch config not found for: " + kernel_name + "_" + launch_call.dim_str() + "_" + std::to_string(cubin_uid));
    ...
    for (auto &config : configs) {
        ...
        GMS_SPD_LOG_ALWAYS("Launch config: " + config.str() + ". Norm speed: " + std::to_string(norm_speed));
    }
    ...
    GMS_SPD_LOG_ALWAYS("Tuning complete ("+ std::to_string(elapsed.count()) + " ms). Launch config: " + best_config.str() + ". Norm speed: " + std::to_string(best_norm_speed) + "\n");
}

void GmsServer::wait_until_launch_queue_empty(int32_t client_id)
{
    int attempt = 0;
...
void GmsServer::client_add_stream(int32_t client_id, cudaStream_t stream)
{
    auto &client_meta = client_data_all[client_id];
...
const void *GmsServer::get_server_addr_from_client_addr(uint32_t client_id, const void *client_addr)
{
...
const void *GmsServer::get_server_addr_from_cu_func(CUfunction cu_func)
{
...
void GmsServer::register_kernel(const void *server_func_addr)
{
    auto &cubin_cache = GmsCache::cache->get_cubin_cache();
...
void GmsServer::register_ptx_transform(uint32_t cubin_uid)
{
...
void GmsServer::handle_cudaLaunchKernel(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{

    GMS_SPD_LOG("Received request: cudaLaunchKernel");
...
void GmsServer::handle_cuLaunchKernel(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuLaunchKernel");
...
void GmsServer::register_cu_module(uint32_t cubin_uid)
{   
    if (cubin_to_cu_module.find(cubin_uid) == cubin_to_cu_module.end()) {
...
void GmsServer::handle___cudaRegisterFatBinary(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: __cudaRegisterFatBinary");
...
void GmsServer::handle___cudaRegisterFunction(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: __cudaRegisterFunction");
...
void GmsServer::handle___cudaRegisterFatBinaryEnd(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: __cudaRegisterFatBinaryEnd");
...
void GmsServer::handle_cuda_allocation_with_mid(cudaMallocResponse* response, cudaMallocArg* args,
                            ...
                            bool reuse_flag = false)
{
    ...
    if (response->err == cudaSuccess) {
        if (reuse_flag) // global
        {
            ...
            GMS_SPD_WARN("Allocated new memory. Current ID counter: {}" + std::to_string(mapped_id)); // Use spdlog's direct formatting
            
        }
        else // client local
        {
            client_dev_addr_map.push_back(mem_region(response->devPtr, args->size));
            GMS_SPD_WARN("Allocated new memory without Current ID counter within client_dev"); // Use spdlog's direct formatting
        }
        // client_dev_addr_map.push_back( mem_region(response->devPtr, args->size));
        ...
        GMS_SPD_WARN("Encountered cudaErrorMemoryAllocation " + std::string(__FILE__) + ":" + std::to_string(__LINE__));
    }
}

void GmsServer::handle_cudaMalloc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudaMalloc");
    ...
    iox_server->loan(requestHeader, sizeof(cudaMallocResponse), alignof(cudaMallocResponse))
        .and_then([&](auto& responsePayload) {
            ...
            else // reuse exist cudaMalloc content - init for memory
            {
                ...
                {
                    // GMS_SPD_WARN("current rc memory id " + std::to_string(client_data_all[client_id].rc_mem_Size));
                    ...
                    if (response->devPtr != nullptr)
                    {
                        ...
                        // {
                            ...
                            GMS_SPD_WARN("recovery current mr size " + std::to_string(client_data_all[client_id].mapped_id));
                        // }
                    }
                    else
                    {
...
void GmsServer::handle_cudaFree(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudaFree");
    auto args = (struct cudaMallocArg *) __args;
    ...
    iox_server->loan(requestHeader, sizeof(cudaFreeArg), alignof(cudaFreeArg))
        .and_then([&](auto& responsePayload) {
            ...
            if(is_registered_addr(dev_addr_map, args->devPtr))
            {
                GMS_SPD_LOG("Bypass cudafree");
                *response = cudaSuccess;
            }
            else
            {
...
void GmsServer::handle_cudaMemcpy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudaMemcpy");
...
void GmsServer::handle_cudaMemcpyAsync(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudaMemcpyAsync");
    ...
    iox_server->loan(requestHeader, res_size, alignof(cudaMemcpyAsyncResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if (args->kind == cudaMemcpyHostToDevice) {
                ...
                {
                    GMS_SPD_LOG("Bypass cudaMemcpyAsync");
                    res->err = cudaSuccess;
                }
                else
                {
...
void GmsServer::handle_cublasSgemm_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasSgemm_v2");
...
void GmsServer::handle_cublasLtMatmul(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasLtMatmul");
...
void GmsServer::handle_cublasLtMatmulDescSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasLtMatmulDescSetAttribute");
...
void GmsServer::handle_cublasLtMatrixLayoutSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasLtMatrixLayoutSetAttribute");
...
void GmsServer::handle_cublasLtMatmulPreferenceSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasLtMatmulPreferenceSetAttribute");
...
void GmsServer::handle_cublasLtMatmulAlgoGetHeuristic(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasLtMatmulAlgoGetHeuristic");
...
void GmsServer::handle_cudnnBackendSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnBackendSetAttribute");
...
void GmsServer::handle_cudnnBackendGetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnBackendGetAttribute");
...
void GmsServer::handle_cudnnActivationForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnActivationForward");
...
void GmsServer::handle_cudnnSetTensorNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSetTensorNdDescriptor");
...
void GmsServer::handle_cudnnSetConvolutionNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSetConvolutionNdDescriptor");
...
void GmsServer::handle_cudnnSetFilterNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSetFilterNdDescriptor");
...
void GmsServer::handle_cudnnConvolutionForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnConvolutionForward");
...
void GmsServer::handle_cudnnGetConvolutionNdForwardOutputDim(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetConvolutionNdForwardOutputDim");
...
void GmsServer::handle_cudnnGetConvolutionForwardAlgorithm_v7(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetConvolutionForwardAlgorithm_v7");
...
void GmsServer::handle_cudnnFindConvolutionForwardAlgorithm(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnFindConvolutionForwardAlgorithm");
...
void GmsServer::handle_cudnnAddTensor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnAddTensor");
...
void GmsServer::handle_cudnnSetPoolingNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSetPoolingNdDescriptor");
...
void GmsServer::handle_cudnnGetPoolingNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetPoolingNdDescriptor");
...
void GmsServer::handle_cudnnGetPoolingNdForwardOutputDim(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetPoolingNdForwardOutputDim");
...
void GmsServer::handle_cudnnPoolingForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnPoolingForward");
...
void GmsServer::handle_cublasSgemv_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasSgemv_v2");
...
void GmsServer::handle_cudnnLRNCrossChannelForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnLRNCrossChannelForward");
...
void GmsServer::handle_cudnnSoftmaxForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSoftmaxForward");
...
void GmsServer::handle_cudnnTransformTensor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnTransformTensor");
...
void GmsServer::handle_cublasSgemmEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasSgemmEx");
...
void GmsServer::handle_cublasGemmEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasGemmEx");
...
void GmsServer::handle_cudnnSetSeqDataDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnSetSeqDataDescriptor");
...
void GmsServer::handle_cudnnGetSeqDataDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetSeqDataDescriptor");
...
void GmsServer::handle_cudnnMultiHeadAttnForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnMultiHeadAttnForward");
...
void GmsServer::handle_cudnnMultiHeadAttnBackwardData(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnMultiHeadAttnBackwardData");
...
void GmsServer::handle_cudnnMultiHeadAttnBackwardWeights(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnMultiHeadAttnBackwardWeights");
...
void GmsServer::handle_cudnnReorderFilterAndBias(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnReorderFilterAndBias");
...
void GmsServer::handle_cudnnGetRNNWorkspaceSize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetRNNWorkspaceSize");
...
void GmsServer::handle_cudnnGetRNNTrainingReserveSize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetRNNTrainingReserveSize");
...
void GmsServer::handle_cudnnGetFilterNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetFilterNdDescriptor");
...
void GmsServer::handle_cudnnRNNForwardTraining(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnRNNForwardTraining");
...
void GmsServer::handle_cudnnRNNBackwardData(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnRNNBackwardData");
...
void GmsServer::handle_cudnnRNNBackwardWeights(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnRNNBackwardWeights");
...
void GmsServer::handle_cudnnSetRNNDataDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnSetRNNDataDescriptor");
...
void GmsServer::handle_cudnnGetTensorNdDescriptor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnGetTensorNdDescriptor");
...
void GmsServer::handle_cudnnBatchNormalizationForwardTrainingEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnBatchNormalizationForwardTrainingEx");
...
void GmsServer::handle_cudnnBatchNormalizationBackwardEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudnnBatchNormalizationBackwardEx");
...
void GmsServer::handle_cublasSgemmStridedBatched(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cublasSgemmStridedBatched");
...
void GmsServer::handle_cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    GMS_SPD_LOG("Received request: cudaOccupancyMaxActiveBlocksPerMultiprocessorWithFlags");
...
void GmsServer::handle_cudaChooseDevice(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaChooseDevice");
...
void GmsServer::handle_cudaSetDevice(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaSetDevice");
...
void GmsServer::handle_cudnnRNNBackwardWeights_v8(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnRNNBackwardWeights_v8");
...
void GmsServer::handle_cudnnRNNBackwardData_v8(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnRNNBackwardData_v8");
...
void GmsServer::handle_cudnnRNNForward(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnRNNForward");
...
void GmsServer::handle_cudnnBackendExecute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnBackendExecute");
...
void GmsServer::handle_cudaThreadSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaThreadSynchronize");
...
void GmsServer::handle_cudaEventRecord(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaEventRecord");
...
void GmsServer::handle_cudaDeviceSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaDeviceSynchronize");
...
void GmsServer::handle_cudaStreamSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamSynchronize");
    ...
    iox_server->loan(requestHeader, sizeof(cudaError_t), alignof(cudaError_t))
        .and_then([&](auto& responsePayload) {
            ...
            if(replay_round[client_data_all[client_id].mapped_id] && !client_data_all[client_id].first_init)
            {
                GMS_SPD_LOG("Bypass cudaStreamSynchronize");
                *response = cudaSuccess;
            }
            else
            {
...
void GmsServer::handle_cublasCreate_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasCreate_v2");
...
void GmsServer::handle_cudnnCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnCreate");
...
void GmsServer::handle_cuModuleLoadData(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleLoadData");
...
void GmsServer::handle_cuModuleGetFunction(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleGetFunction");
...
void GmsServer::handle_cuPointerGetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuPointerGetAttribute");
...
void GmsServer::handle_cudaStreamGetCaptureInfo_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamGetCaptureInfo_v2");
...
void GmsServer::handle_cudaGraphGetNodes(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaGraphGetNodes");
...
void GmsServer::handle_cudaFuncSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaFuncSetAttribute");
...
void GmsServer::handle_cuGetProcAddress_v2(void*, iox::popo::UntypedServer*, void const*)
{
    GMS_SPD_LOG("Received request: cuGetProcAddress_v2");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cuMemcpy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemcpy");
...
void GmsServer::handle_cuMemcpyAsync(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemcpyAsync");
...
void GmsServer::handle_cuMemAllocAsync(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemAllocAsync");
...
void GmsServer::handle_cuMemFree_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemFree_v2");
...
void GmsServer::handle_cudaMemset(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaMemset");
    ...
    iox_server->loan(requestHeader, sizeof(cudaError_t), alignof(cudaError_t))
        .and_then([&](auto& responsePayload) {
            ...
            if(replay_round[client_data_all[client_id].mapped_id] && !client_data_all[client_id].first_init)
            {
                GMS_SPD_LOG("Bypass cudaMemsetAsync");
                *response = cudaSuccess;
            }
            else
            {
...
void GmsServer::handle_cudaStreamCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamCreate");
...
void GmsServer::handle_cudaStreamCreateWithFlags(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamCreateWithFlags");
...
void GmsServer::handle_cudaStreamCreateWithPriority(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamCreateWithPriority");
...
void GmsServer::handle_cudaStreamBeginCapture(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamBeginCapture");
...
void GmsServer::handle_cuStreamCreateWithPriority(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamCreateWithPriority");
...
void GmsServer::handle_cuFuncGetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuFuncGetAttribute");
	auto args = (struct cuFuncGetAttributeArg *) __args;
	auto requestHeader = iox::popo::RequestHeader::fromPayload(requestPayload);

	auto server_func_addr = get_server_addr_from_cu_func(args->hfunc);
	auto cu_func_original = GmsServer::server->original_kernel_map[server_func_addr].func;
...
void GmsServer::handle_cuFuncSetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuFuncSetAttribute");
	auto args = (struct cuFuncSetAttributeArg *) __args;
	auto requestHeader = iox::popo::RequestHeader::fromPayload(requestPayload);

	auto server_func_addr = get_server_addr_from_cu_func(args->hfunc);
	auto cu_func_original = GmsServer::server->original_kernel_map[server_func_addr].func;
	auto cu_func_ptb = GmsServer::server->ptb_kernel_map[server_func_addr].func;
	auto cu_func_dynamic = GmsServer::server->dynamic_ptb_kernel_map[server_func_addr].func;
	auto cu_func_preemptive = GmsServer::server->preemptive_ptb_kernel_map[server_func_addr].func;
    auto cu_func_sliced = GmsServer::server->sliced_kernel_map[server_func_addr].func;
...
void GmsServer::handle_cuFuncSetCacheConfig(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuFuncSetCacheConfig");
...
void GmsServer::handle_cublasGemmStridedBatchedEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasGemmStridedBatchedEx");
...
void GmsServer::handle_cublasGemmBatchedEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasGemmBatchedEx");
	auto args = (struct cublasGemmBatchedExArg *) __args;
    auto msg_header = static_cast<const MessageHeader_t*>(requestPayload);
    int32_t client_id = msg_header->client_id;

    auto partial_and_args = cublasGemmBatchedEx_Partial(args);
...
void GmsServer::handle_cuMemsetD8_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemsetD8_v2");
...
void GmsServer::handle_cuStreamCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamCreate");
...
void GmsServer::handle_cuMemAlloc_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemAlloc_v2");
...
void GmsServer::handle_cuMemsetD32_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemsetD32_v2");
...
void GmsServer::handle_cuMemcpyHtoDAsync_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemcpyHtoDAsync_v2");
...
void GmsServer::handle_cuMemcpyDtoHAsync_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemcpyDtoHAsync_v2");
...
void GmsServer::handle_cuMemsetD32Async(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemsetD32Async");
...
void GmsServer::handle_cuMemcpyDtoDAsync_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemcpyDtoDAsync_v2");
...
void GmsServer::handle_cuModuleLoadFatBinary(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleLoadFatBinary");
...
void GmsServer::handle_cuModuleLoadDataEx(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleLoadDataEx");
...
void GmsServer::handle_cuModuleGetGlobal_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleGetGlobal_v2");
...
void GmsServer::handle_cuCtxSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuCtxSynchronize");
...
void GmsServer::handle_cuStreamSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamSynchronize");
...
void GmsServer::handle_cuModuleUnload(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuModuleUnload");
...
void GmsServer::handle_cudaStreamEndCapture(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamEndCapture");
...
void GmsServer::handle_cuStreamEndCapture(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamEndCapture");
...
void GmsServer::handle_cuGraphLaunch(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuGraphLaunch");
...
void GmsServer::handle_cublasSetMathMode(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasSetMathMode");
...
void GmsServer::handle_cublasDestroy_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasDestroy_v2");
...
void GmsServer::handle_cublasSetStream_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasSetStream_v2");
...
void GmsServer::handle_cublasSetWorkspace_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasSetWorkspace_v2");
...
void GmsServer::handle_cublasLtCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtCreate");
...
void GmsServer::handle_cublasLtMatmulDescCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatmulDescCreate");
...
void GmsServer::handle_cublasLtMatrixLayoutCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatrixLayoutCreate");
...
void GmsServer::handle_cublasLtMatmulPreferenceCreate(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatmulPreferenceCreate");
...
void GmsServer::handle_cudaPointerGetAttributes(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaPointerGetAttributes");
...
void GmsServer::handle_cuDevicePrimaryCtxSetFlags_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuDevicePrimaryCtxSetFlags_v2");
...
void GmsServer::handle_cudaMallocHost(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaMallocHost");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cuMemHostAlloc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuMemHostAlloc");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cudaHostAlloc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaHostAlloc");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cudaFreeHost(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaFreeHost");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cuDeviceGetName(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuDeviceGetName");
...
void GmsServer::handle_cudaFuncGetAttributes(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaFuncGetAttributes");
...
void GmsServer::handle_cudnnGetErrorString(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnGetErrorString");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_ncclCommInitRankConfig(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: ncclCommInitRankConfig");
...
void GmsServer::handle_cudaGetDevice(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaGetDevice");
...
void GmsServer::handle_cuDevicePrimaryCtxGetState(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuDevicePrimaryCtxGetState");
...
void GmsServer::handle_cublasLtMatrixLayoutDestroy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatrixLayoutDestroy");
...
void GmsServer::handle_cublasLtMatmulDescDestroy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatmulDescDestroy");
...
void GmsServer::handle_cudaGetLastError(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaGetLastError");
...
void GmsServer::handle_cublasLtDestroy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtDestroy");
...
void GmsServer::handle_cublasGetMathMode(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasGetMathMode");
...
void GmsServer::handle_cublasLtMatmulPreferenceDestroy(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cublasLtMatmulPreferenceDestroy");
...
void GmsServer::handle_cudaMemsetAsync(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaMemsetAsync");
...
void GmsServer::handle_cudaPeekAtLastError(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaPeekAtLastError");
...
void GmsServer::handle_cuCtxGetApiVersion(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuCtxGetApiVersion");
...
void GmsServer::handle_cuCtxGetDevice(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuCtxGetDevice");
...
void GmsServer::handle_cuCtxGetCurrent(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuCtxGetCurrent");
...
void GmsServer::handle_cuCtxCreate_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuCtxCreate_v2");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cudaDeviceGetAttribute(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaDeviceGetAttribute");
...
void GmsServer::handle_cuGraphInstantiateWithFlags(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuGraphInstantiateWithFlags");
    ...
    iox_server->loan(requestHeader, sizeof(cuGraphInstantiateWithFlagsResponse), alignof(cuGraphInstantiateWithFlagsResponse))
        .and_then([&](auto& responsePayload) {
            ...
            GMS_SPD_WARN("Failing cuGraphInstantiateWithFlags intentionally");
...
void GmsServer::handle_cuGetExportTable(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuGetExportTable");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}

void GmsServer::handle_cudaStreamIsCapturing(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaStreamIsCapturing");
...
void GmsServer::handle_cuStreamBeginCapture_v2(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamBeginCapture_v2");
...
void GmsServer::handle_cuStreamIsCapturing(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cuStreamIsCapturing");
...
void GmsServer::handle_cudnnReduceTensor(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudnnReduceTensor");
...
void GmsServer::handle_cudaLaunchHostFunc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
	GMS_SPD_LOG("Received request: cudaLaunchHostFunc");
	throw std::runtime_error(std::string(__FILE__) + ":" + std::to_string(__LINE__) + ": Unimplemented.");
}
