...
{
    return get_gms_home_dir() / "build";
}

std::filesystem::path get_gms_home_dir()
{
    if (std::getenv("GMS_HOME")) {
        return std::filesystem::path(std::string(std::getenv("GMS_HOME")));
    } else {
        return std::filesystem::path(std::string(std::getenv("HOME"))) / "gms";
    }
}
