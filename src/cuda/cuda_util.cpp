...
std::vector<std::string> gen_ptx_from_cubin(std::string cubin_path)
{
    ...
    for (auto &capability : candidate_cuda_compute_capabilities) {
        ...
        if (ptx_file_names.empty()) {
            // GMS_SPD_WARN("Fail to find ptx code from " + cubin_path + " for compute capability " + capability);
            has_failed = true;
            continue;
        }

        if (has_failed) {
            GMS_SPD_WARN("Fall back to use PTX code for compute capability " + capability);
        }
