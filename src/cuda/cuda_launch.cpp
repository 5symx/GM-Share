...
std::vector<CudaLaunchConfig> CudaLaunchConfig::get_workload_agnostic_sharing_configs(CudaLaunchCall &launch_call)
{
    ...
    auto kernel_name = GmsServer::server->host_func_to_demangled_kernel_name_map[launch_call.func];
...
// auto kernel_name = GmsServer::server->host_func_to_demangled_kernel_name_map[func];
    ...
    PTBKernelArgs *ptb_args, uint32_t *curr_idx_arr, SlicedKernelArgs *slice_args)
{
    ...
    if (use_original) {
        kernel_map_ptr = &GmsServer::server->original_kernel_map;
    } else if (use_ptb) {
        kernel_map_ptr = &GmsServer::server->ptb_kernel_map;
    } else if (use_dynamic_ptb) {
        kernel_map_ptr = &GmsServer::server->dynamic_ptb_kernel_map;
    } else if (use_preemptive_ptb) {
        kernel_map_ptr = &GmsServer::server->preemptive_ptb_kernel_map;
    } else if (use_sliced) {
        kernel_map_ptr = &GmsServer::server->sliced_kernel_map;
    }
